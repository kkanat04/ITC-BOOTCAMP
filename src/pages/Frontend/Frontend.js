...
const Frontend = ({ThanksModalForCourses, setThanksModalForCourses, endModal, openingModalHandler, setEndModal}) => {
    let state = {
        about: ['Научиться проектировать интересный, привлекательный и удобный интерфейс',
            'Научиться создавать адаптивный дизайн',
            'Научиться логике и алгоритму языка программирования Javascript, чтобы решать любые поставленные задачи в проектах',
            'Работать с современной библиотекой React JS',
            'Сделать портфолио из реальных проектов'],
        titles: [
            {
                name: 'HTML,CSS',
                array: [
    ...
    return (
        <div className={transitionFront === true ? 'main__opacity' : 'main__opacity-time'}>
            ...
            <FrontBackground front OpenModal={OpenModal} setOpenModal={setOpenModal}
                             ThanksModalForCourses={ThanksModalForCourses}
                             setThanksModal={setThanksModal} ThanksModal={ThanksModal}
                             setThanksModalForCourses={setThanksModalForCourses}/>

            <About front state={state.about}/>
            <Tech front/>
            <Target front/>
            <Dropdown front state={state.titles}/>
            <Diploma front state={state.resume}/>
            <Process OpenModal={OpenModal} setOpenModal={setOpenModal}/>
            <Consult ThanksModal={ThanksModal} setThanksModal={setThanksModal} open={openingModalHandler}
                     setThanksModalForCourses={setThanksModalForCourses} ThanksModalForCourses={ThanksModalForCourses}/>
